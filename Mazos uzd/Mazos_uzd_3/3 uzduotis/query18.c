...
int main(int argc, char *argv[]) {
    ...
    if (db){
        ...
        for (int i = 0; i < size; ++i){ // process all the student records in database
            ...
            int name_unique = 1;
            int surname_unique = 1;
            for(int i2 = 0; i2 < size; i2++)
            {
                if(name_unique == 1 && i != i2 && strcmp(s.name, students[i2].name) == 0)
                {
                    name_unique = 0;
                }
                if(surname_unique == 1 && i != i2 && strcmp(s.surname, students[i2].surname) == 0)
                {
                    surname_unique = 0;
                }
            }
            //Print student data if the student isn't filtered
            if(name_unique || surname_unique)
            {
                counter++;
