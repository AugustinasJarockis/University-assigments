...
int main(int argc, char *argv[]) {
    ...
    if (db){
        ...
        //Recalculate all averages correctly
        for(int i = 0; i < size; i++)
        {
            ...
            {
                sum += students[i].grades[i2];
            }
            students[i].average = sum / ((double)s.load);
        }
        //Count students for all courses
        for(int i = 0; i < size; i++)
        {
            if(students[i].average > highest_mark)
                highest_mark = students[i].average;
        }
        for (int i = 0; i < size; ++i){ // process all the student records in database
            int student_ok = 0;
            Student s = students[i]; // store data for each student in s
            //Find if student is close to the best student
            if(s.average >= highest_mark - 1)
                student_ok = 1;
